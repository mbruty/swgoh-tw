...
function merge(main, start, mid, end, copy) {
  ...
  while (i <= mid && j <= end){
      let l = 0;
      while(copy[i][0].squad.title.charCodeAt(l) === copy[j][0].squad.title.charCodeAt(l)){
        l++;
      }
      if (copy[i][0].squad.title.charCodeAt(l) <= copy[j][0].squad.title.charCodeAt(l) ) main[k++] = copy[i++];
      else main[k++] = copy[j++];
  }
